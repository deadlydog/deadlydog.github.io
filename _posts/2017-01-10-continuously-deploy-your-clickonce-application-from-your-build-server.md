...
If you like you can now run the build and see if it succeeds. If the build fails with an error relating to an expired certificate or pfx file, see [my other blog post on importing the required certificate on the build server at build-time](http://dans-blog.azurewebsites.net/creating-a-pfx-certificate-and-applying-it-on-the-build-server-at-build-time/), which involves adding one more "Import-PfxCertificate.ps1" build step before the MSBuild step.

---
title: "Increase app fault tolerance with the Stale Cache pattern"
permalink: /Increase-app-fault-tolerance-with-the-Stale-Cache-pattern/
...
When used properly, it can also increase fault tolerance.
In this post, I will introduce the `Stale Cache` pattern and show how to use it to increase the fault tolerance of your applications.
...
This pattern has helped prevent production outages in several applications I've worked on, and I hope it can help you too.
...
- Where in your application stack should you do the caching? In the client application, web server, database, or a separate component such as a Redis server or CDN.
...
In this post, I'm focusing on having your application remain operational when an external service that it depends on is unavailable, such as a database or web service.
...
Fresh bread always makes a sandwich taste way better, so you toss the stale bread in the garbage and go to the bakery to get some fresh bread.
...
Now you are home, and with no bread you are unable to make a sandwich.

You might say, well just pull the stale bread out of the garbage and make a sandwich with that.
Even if you are the type of person to pull food out of the garbage ([like George Costanza](https://www.youtube.com/watch?v=t36jwyVncmQ)), in this hypothetical scenario let's say that your roommate already _collected the garbage_ and took it out (see what I did there 😛), so the bread really is gone.
...
You likely still want to remove stale items from the cache eventually though, as using data that is too old may cause other problems in your application.
Just like how you wouldn't want to make a sandwich with bread that was molding, you wouldn't want to use data that is too old.
...
When your roommate returns with the fresh bread, you can throw out the stale bread and make your 2nd sandwich with the fresh bread.
This is another way to implement the Stale Cache pattern, where you return the stale cache item right away and request a fresh item to update the cache with in the background.


--- TODO: Add a picture of the async Stale Cache pattern here ---


With the Stale Cache pattern, there are 3 states for a cache item:

- Fresh: The item has not yet reached its Stale (or best-before) time.
  The item is not stale yet and can be used without any additional actions.
- Stale: The item has passed its Stale time, but not its Expiry time.
...
- Expired: The item has passed both its Stale and Expiry time.
...
There are several different ways the Stale Cache pattern may be implemented, each
...
For example, if you are using a cache-aside strategy, when you discover that the item returned from the cache is stale, you could request a fresh item from the source and then update the cache with the fresh item.
...
For example, you may return a stale cache item while also retrying failed requests to the source in the background.
...
Another important consideration is that the Stale Cache pattern is only applicable to read operations, not write operations.
...
- Product prices (potentially, if the business values making a sale over selling the item at an slightly older price)
...
Below is a basic example of implementing the Stale Cache pattern in C#.
I build upon the .NET MemoryCache class.
...
public class StaleMemoryCache
{
  ...
  public void Set<T>(object key, T value, TimeSpan timeBeforeItemIsStale, TimeSpan timeBeforeItemExpires)
  {
    var cacheItem = new CacheItem<T>
    {
      Value = value,
      StaleDate = DateTimeOffset.UtcNow.Add(timeBeforeItemIsStale)
    };
    var expiryDate = DateTimeOffset.UtcNow.Add(timeBeforeItemExpires);
...
- To to set an item in the cache you must provide _two_ times; the time before the item is considered stale, and the time before the item expires (is removed from the cache).
  Caches typically only allow you to specify the expiry time.
...
I hope this pattern becomes standard functionality in all caching libraries.
The only place I've seen it used so far is
Until then, I hope you find it useful in your applications.
